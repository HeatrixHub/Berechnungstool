...
from ..core.flow_calculations import compute_flow_properties
...
def create_tab2(notebook):
    ...
    flow_unit_dropdown = ttk.Combobox(
        frame_tab2,
        textvariable=flow_unit_var,
        values=["m³/h", "m³/s"],
        state="readonly",
        width=5,
    )
    ...
    def calculate():
        ...
        side_b = None
        if shape_var.get() == "Rund":
            diameter = check_float(entries["Durchmesser (mm):"])
            if diameter is None:
                return
        else:
            side_a = check_float(entries["Seite a (mm):"])
            side_b = check_float(entries["Seite b (mm):"])
            if side_a is None or side_b is None:
                return

        temperature = _optional_float(entries["Temperatur (°C):"])
        density = _optional_float(entries["Dichte (kg/m³):"])
        ...
        if result["reynolds"] is not None:
            ...
            entry_reynolds.config(state="normal")
            entry_reynolds.delete(0, tk.END)
            entry_reynolds.config(state="readonly")
            entry_flowtype.config(state="normal")
            entry_flowtype.delete(0, tk.END)
            entry_flowtype.config(state="readonly")
